...
if use_state_separation:
    ...
    writable_root = platform_appdata_path / 'ArknightsAutoHelper.PcrInventory'
...
def _get_instance_id_win32():
    ...
    i = 0
    while True:
        name = 'Global\\ArknightsAutoHelper.PcrInventory.%08X.%d' % (path_hash, i)
        import ctypes
...
def _get_instance_id_posix():
    ...
    while True:
        try:
            if i == 0:
                filename = Path.joinpath(logs, 'pcrhelper.log')
            else:
                filename = Path.joinpath(logs, 'pcrhelper.%d.log' % i)
            f = open(filename, 'a+b')
            f.seek(0)
            import fcntl
...
def get_instance_id():
    ...
    if _instanceid == 0:
        logfile = Path.joinpath(logs, 'pcrhelper.log')
    else:
        logfile = Path.joinpath(logs, 'pcrhelper.%d.log' % _instanceid)
...
def enable_logging(extra_handlers: Optional[Sequence[logging.Handler]] = None):
    ...
    logging.debug('pcrhelper version %s', version)
    import coloredlogs
    coloredlogs.install(
        fmt=' Ξ %(message)s',
